 ...
 */
	public function confirm_transactions($id) {
		...
		$file_url="../webroot/statements/bank_".($_bank_id)."/".($_year)."/$_file_name";
	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if(isset($_FILES['fileField']['name']) and !empty($_FILES['fileField']['name'])){//getting the file extension and testing it
				...
				//uploading the file
				if(move_uploaded_file($_FILES['fileField']['tmp_name'],"../webroot/statements/bank_".($this->Auth->User('bank_id'))."/".(date("Y"))."/$file_name")){
					...
					if ($this->PaymentStatement->save($this->request->data)) {
						...
						@unlink("../webroot/statements/bank_".($this->Auth->User('bank_id'))."/".(date("Y"))."/$file_name");
 ...
 */
	public function delete($id = null) {
		...
		if ($this->PaymentStatement->delete()) {
			...
			if(file_exists("../webroot/statements/bank_".($f[0]['PaymentStatement']['bank_id'])."/".($f[0]['PaymentStatement']['year'])."/".($f[0]['PaymentStatement']['file_name']))){
				@unlink("../webroot/statements/bank_".($f[0]['PaymentStatement']['bank_id'])."/".($f[0]['PaymentStatement']['year'])."/".($f[0]['PaymentStatement']['file_name']));
			}
