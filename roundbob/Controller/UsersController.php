	...
	public function get_users($q=null,$selected_ids=-2){
		...
		$this->paginate=array(
			'conditions'=>array(
				...
				'NOT'=>array('User.id'=>array(addslashes(strip_tags($selected_ids)))),
			)
		);
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if(isset($_FILES['fileField']['name']) and !empty($_FILES['fileField']['name'])){//getting the file extension and testing it
				...
				if(!$FileExtensionAllowed){
					...
					if(file_exists("../webroot/img/pic/$previous_profile_image")){
						if($this->request->data['User']['profile_image']!='default.png')
							@unlink("../webroot/img/pic/$previous_profile_image");
					}
					
					if(move_uploaded_file($_FILES['fileField']['tmp_name'],"../webroot/img/pic/$file_name")){
						$this->request->data['User']['profile_image']=$file_name;//filename to use for submitted file
						@$this->ImageResize->resize("../webroot/img/pic/$file_name", 300, 300);						
					}
            ...
            if ($this->User->save($this->request->data)) {
				...
				if(file_exists("../webroot/img/pic/$file_name")){
					@unlink("../webroot/img/pic/$file_name");
				}
