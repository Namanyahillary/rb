 ...
 */
	public function bob_admin_add() {
		if ($this->request->is('post')) {
			...
			//uploading the file
			if(move_uploaded_file($_FILES['fileField']['tmp_name'],"../webroot/img/destinations/$file_name")){
				$this->request->data['Destination']['image_file']=$file_name;//filename to use for submitted file
				
				if (copy("../webroot/img/destinations/$file_name","../webroot/img/imagecache/destinations/$file_name")) {
					$image ="../webroot/img/imagecache/destinations/$file_name"; 
					...
					if ($this->Destination->save($this->request->data)) {
						...
						if(file_exists("../webroot/img/destinations/$file_name")){
							@unlink("../webroot/img/destinations/$file_name");
						}
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if(isset($_FILES['fileField']['name']) and !empty($_FILES['fileField']['name'])){//getting the file extension and testing it
				...
				if(!$FileExtensionAllowed){
					...
					if(file_exists("../webroot/img/destinations/$previous_profile_image")){
						if($this->request->data['Destination']['image_file']!='default.png')
							@unlink("../webroot/img/destinations/$previous_profile_image");
					}
					
					if(file_exists("../webroot/img/imagecache/destinations/$previous_profile_image")){
							@unlink("../webroot/img/imagecache/destinations/$previous_profile_image");
					}
