		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if(isset($_FILES['fileField']['name']) and !empty($_FILES['fileField']['name'])){//getting the file extension and testing it
				...
				if(!$FileExtensionAllowed){
					...
					if(file_exists("../webroot/img/group_covers/$previous_profile_image")){
						if($previous_profile_image!='default.png')
							@unlink("../webroot/img/group_covers/$previous_profile_image");
					}
					
					if(file_exists("../webroot/img/imagecache/group_covers/$previous_profile_image")){
							@unlink("../webroot/img/imagecache/group_covers/$previous_profile_image");
					}
					
					if(move_uploaded_file($_FILES['fileField']['tmp_name'],"../webroot/img/group_covers/$file_name")){
						...
						if (copy("../webroot/img/group_covers/$file_name","../webroot/img/imagecache/group_covers/$file_name")) {
							$image ="../webroot/img/imagecache/group_covers/$file_name"; 
	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if ($this->Group->save($this->request->data)) {
				...
				$this->request->data['GroupMember']['group_id']=$this->Group->getInsertID();
				$this->request->data['GroupMember']['id']=($this->Group->getInsertID()).'_'.($this->Auth->User('id'));
				$this->request->data['GroupMember']['user_id']=($this->Auth->User('id'));					
